                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/50 uppercase tracking-widest">
                          {t.format} • {t.type} • {t.status === 'finished' ? 'Completed' : 'Ongoing'}
                        </p>
...
function TournamentDetail({ tournament, onBack, onPlayMatch, playingMatchId, renderMatchDashboard }: { tournament: Tournament, onBack: () => void, onPlayMatch: (matchId: string) => void, playingMatchId?: string | null, renderMatchDashboard?: (matchId: string) => React.ReactNode }) {
  ...
  const generateMatches = async () => {
    ...
    const baseMatch = {
      ...
      status: 'ongoing',
      created_at: new Date().toISOString(),
      total_overs: tournament.format === 't20' ? 20 : tournament.format === 'odi' ? 50 : 90,
  ...
  const standings = useMemo(() => {
    ...
    return teams.map(team => {
      ...
      const wins = finishedMatches.filter(m => m.winner_id === team.id).length;
      ...
      finishedMatches.forEach(m => {
        const isTeamABattingFirst = (m.toss_winner_id === m.team_a_id && m.toss_decision === 'bat') ||
                                   (m.toss_winner_id === m.team_b_id && m.toss_decision === 'bowl');
        
        const teamBattingInnings = (m.team_a_id === team.id) 
          ? (isTeamABattingFirst ? 1 : 2) 
          : (isTeamABattingFirst ? 2 : 1);
          
        const teamBowlingInnings = teamBattingInnings === 1 ? 2 : 1;

        const matchBalls = balls.filter(b => b.match_id === m.id);
        
        const battingBalls = matchBalls.filter(b => b.innings_no === teamBattingInnings);
        const bowlingBalls = matchBalls.filter(b => b.innings_no === teamBowlingInnings);
